...
class AuditorAction(Base):
    """
    AuditorAction Model
    Stores an audit trail of every investigator action taken on an alert.
    """
    __tablename__ = 'auditor_actions'

    id = Column(Integer, primary_key=True, autoincrement=True)
    emp_id = Column(String(255), nullable=False, index=True)
    action = Column(String(50), nullable=False)
    timestamp = Column(DateTime, default=func.now(), nullable=False)
    investigator_id = Column(String(255), nullable=False)
