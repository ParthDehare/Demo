...
export default function App() {
  ...
  const [vaultEvidence, setVaultEvidence] = useState(() => {
    return Array.from({ length: 10 }).map((_, i) => ({
      id: `EVD-20260516${100000 + i}`,
      emp_id: `EMP_${1000 + i}`,
      filename: `EVD-20260516${100000 + i}.pdf`,
      hash: `0x${Math.random().toString(16).slice(2, 10)}...${Math.random().toString(16).slice(2, 6)}`,
      blockId: `#${894000 + i}`,
      timestamp: `2026-05-1${6 - (i % 3)}T1${i}:00:00Z`,
      status: "Generated"
    }));
  });
  ...
  // 2. Live Stream (Delta Update) - crash-safe
  ...
  // Auto-refresh interval for live stream (2000ms)
  useEffect(() => {
  ...
  }, [autoRefresh, fetchNextTransaction]);
