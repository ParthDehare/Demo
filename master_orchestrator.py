...
class VaultMindOrchestrator:
    ...
    def process_transaction(self, tx_data: dict) -> dict:
        ...
        # ── AGENT 8: DeceptionGuard (Honeypot Override) ──
        dest_acc = tx_data.get('destination_account', '')
        a8_result = self.agent8.evaluate_access(dest_acc)
        if a8_result['risk_score'] > 0:
            aggregated_score = max(aggregated_score, 100.0)
            signals_triggered.append(f"DeceptionGuard: {a8_result['evidence']['trigger']}")
            
        # ── AGENT 3: TemporalGuard (Velocity) ──
        emp_id = tx_data.get('emp_id')
        a3_score, a3_reason = self.agent3.check_velocity(emp_id, tx_data.get('timestamp'))
        if a3_score > 0:
            aggregated_score += a3_score * 0.4
            signals_triggered.append(f"TemporalGuard: {a3_reason}")
            
        # ── AGENT 5: ProfileAudit (RBAC) ──
        a5_score, a5_reason = self.agent5.audit_profile(
            tx_data.get('emp_class', 'CLERK'),
            tx_data.get('action_type', ''),
            tx_data.get('amount', 0)
        )
        if a5_score > 0:
            aggregated_score = max(aggregated_score, a5_score)
            signals_triggered.append(f"ProfileAudit: {a5_reason}")

        # ── AGENT 6: RegulatoryAI (Compliance Limits) ──
        channel = tx_data.get('transfer_channel', 'NEFT')
        a6_score, a6_reason = self.agent6.check_compliance(tx_data.get('amount', 0), channel)
        if a6_score > 0:
            aggregated_score = max(aggregated_score, a6_score)
            signals_triggered.append(f"RegulatoryAI: {a6_reason}")

        # ── AGENT 4: SentimentWatch (NLP Text Analysis) ──
        remarks = tx_data.get('remarks', '')
        if remarks:
            a4_score, a4_source = self.agent4.analyze_text(remarks)
            if a4_score >= 50:
                aggregated_score += (a4_score * 0.5)
                signals_triggered.append(f"SentimentWatch: High risk intent detected via {a4_source} (Score: {a4_score})")

        # ── AGENT 1 & 2: Machine Learning Intelligence ──
        if self.a1_model and aggregated_score < 100:
            features = [[
            ...
            a1_pred = self.a1_model.predict(features_scaled)[0]
            if a1_pred == -1:
                aggregated_score += 25
                signals_triggered.append("BehaviourWatch: Anomalous behavioral footprint detected.")

        if self.a2_model and aggregated_score < 100:
            ...
            if src_acc in self.a2_account_mapping and dest_acc in self.a2_account_mapping:
                ...
                edge_feat = self.a2_edge_scaler.transform([[tx_data.get('amount', 0), tx_data.get('dwell_time_seconds', 0)]])
                ...
                if prob > 0.8:
                    aggregated_score += 35
                    signals_triggered.append(f"FundFlow GNN: Malicious edge pattern detected (Prob: {prob:.2f})")
